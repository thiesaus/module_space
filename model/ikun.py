...
class Model_IKUN(nn.Module):
    ...
    def forward(self, x, epoch=1e5):
        output = dict()
        x['exp']=tokenize(x['sentences']).cuda()
        textual_hidden, textual_feat = self.textual_encoding(x['exp'])
        if self.opt['kum_mode'] and (epoch >=0):
            if self.opt['kum_mode'] == 'cascade attention':
                visual_feat = self.visual_local_global(
                    x['local_images'], x['global_image'], textual_hidden, self.opt['kum_mode']
                )
            elif self.opt['kum_mode'] in ['cross correlation', 'text-first modulation']:
                visual_feat = self.visual_local_global(
                    x['local_images'], x['global_image'], textual_feat, self.opt['kum_mode']
                )
        else:
    ...
    def textual_encoding(self, tokens):
        x_hidden, x = self.clip.encode_text_2(tokens, 10)
        x = self.text_fc(x)
        if self.training:
            return x_hidden, x
        else:
    ...
    def get_img_fc(self, use_ln=True):
        if use_ln:
            ...
            return nn.Linear(self.img_dim, 1024)
...
def build_modelIKUN(config: dict):
    config['kum_mode'] = 'cascade attention'
    model = Model_IKUN(config)
