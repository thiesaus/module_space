...
from model.enrich_model import EnrichModel,build_enrich
...
def train(config: dict):
    ...
    model = build_enrich(config=config)
    ...
    if config["WANDB"]:
        ...
        name=f"enrich_"+str(config["NUM_LAYERS"])+"_layers", 
...
def train_one_epoch(model: EnrichModel, train_states: dict, max_norm: float,
...
def test_one_epoch(model:EnrichModel,dataloader_test: DataLoader,epoch):
